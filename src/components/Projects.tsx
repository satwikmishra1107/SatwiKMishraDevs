...
export default function Projects() {
  return (
    ...
    >
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-cyan-950/10 to-purple-950/10 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        {/* Alternating Row Structure: Texts Left, Cards Right */}
        <div className="space-y-24 md:space-y-32">
          {PROJECTS.map((project, index) => {
            const isFirst = index === 0;
            
            // Mock statistical data for deep technical context
            const analysisStats = isFirst ? [
            ...
            return (
              ...
              >
                {/* Left Side: Statistical Data, Explanatory Text, and Main Actions */}
                <div className="lg:col-span-6 order-2 lg:order-1 space-y-6">
                  <div className="flex items-center gap-2 font-mono text-[10px]">
                    <span className={`w-1.5 h-1.5 rounded-full ${isFirst ? 'bg-cyan-400' : 'bg-purple-400'} animate-pulse`} />
                  ...
                  {/* Comprehensive Statistics Text Area */}
                  <div className="grid grid-cols-2 gap-4 bg-neutral-950/40 p-5 rounded-xl border border-neutral-900/80 backdrop-blur-sm">
                      ...
                      <div key={sIdx} className="border-b border-neutral-900/60 pb-2 last:border-0 font-mono">
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                          ...
                          <span className={`text-[8px] font-medium ${
                            ...
                            stat.variance.includes('-') ? 'text-cyan-400' : 'text-neutral-500'
                          }`}>
                            {stat.variance}
                          </span>
                        </div>
                  ...
                  {/* High Visibility Action Buttons directly underneath text area */}
                  <div className="flex flex-wrap sm:flex-nowrap gap-4 pt-2 font-mono text-xs">
                    <a
                      href={project.demoUrl}
                      target="_blank"
                      rel="noreferrer"
                      className={`flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 rounded-lg border font-semibold tracking-wider backdrop-blur-md transition-all duration-300 shadow-md ${
                        isFirst 
                          ? 'bg-cyan-950/30 border-cyan-500/40 text-cyan-400 hover:bg-cyan-500/10 hover:border-cyan-400 hover:shadow-cyan-glow/20' 
                          : 'bg-purple-950/30 border-purple-500/40 text-purple-400 hover:bg-purple-500/10 hover:border-purple-400 hover:shadow-purple-glow/20'
                      }`}
                    >
                ...
                {/* Right Side: The Project Cards */}
                <div className="lg:col-span-6 order-1 lg:order-2 flex justify-center lg:justify-end">
                  <div
                    className="group relative w-full max-w-[540px] rounded-xl bg-black/45 border border-neutral-900 backdrop-blur-md overflow-hidden transition-all duration-500 hover:border-cyan-500/50 hover:shadow-cyan-glow/20 flex flex-col justify-between"
                    style={{
                  ...
                  >
                    ...
                    <div
                      className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none ${
                        isFirst ? 'shadow-cyan-glow' : 'shadow-purple-glow'
                      }`}
                    ...
                    <div className="relative aspect-[16/10] w-full overflow-hidden border-b border-neutral-900">
                      ...
                      {/* Icon HUD banner */}
                      <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-md border border-neutral-800/80 rounded px-2.5 py-1 flex items-center gap-1.5 font-mono text-[9px] text-neutral-400">
                        {isFirst ? (
                    ...
                    <div className="p-6 md:p-8 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        {/* Title */}
                        <h3 className="font-display text-2xl font-bold text-white mb-3 group-hover:text-cyan-400 transition-colors duration-300">
                          {project.title}
                        </h3>
                      ...
                      <div>
                        <div className="grid grid-cols-3 gap-2 bg-neutral-950/60 p-4 rounded-lg border border-neutral-900 font-mono text-center">
                          {project.stats.map((stat) => (
                            <div key={stat.label}>
                              ...
                              <div className={`text-xs font-semibold mt-1 ${isFirst ? 'text-cyan-400' : 'text-purple-400'}`}>
                                {stat.value}
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="mt-4 pt-4 border-t border-neutral-900/50 font-mono text-[10px] text-neutral-500">
